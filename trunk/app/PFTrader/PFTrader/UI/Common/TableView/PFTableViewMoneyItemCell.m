...
-(void)reloadDataWithItem:( PFTableViewItem* )item_
{
   ...
   NSString* money_value_ = [ NSString stringWithMoney: money_item_.amount ];

   self.valueLabel.text = [ money_value_ stringByAppendingFormat: @" %@", money_item_.currency ];

   if ( money_item_.colorSign )
   {
      self.valueLabel.textColor = money_item_.amount >= 0.0
         ? [ UIColor positivePriceColor ]
         : [ UIColor negativePriceColor ];
   }
   else
   {
      self.valueLabel.textColor = [ UIColor colorWithWhite: 0.78f alpha: 1.f ];
   }
}
