...
-(void)reloadDataOrReuse:( BOOL )reuse_
{
   ...
   insets_.top = [ self.dataSource heightOfHeaderInGridView: self ];
   insets_.bottom = [ self.delegate heightOfFooterInGridView: self ];
   
   self.verticalScrollView.contentInset = insets_;
   self.verticalScrollView.scrollIndicatorInsets = insets_;
   ...
   //! need check modification. In old case, when reloadData - horisontal scroll offset resets.
   //self.activePage = 0;
