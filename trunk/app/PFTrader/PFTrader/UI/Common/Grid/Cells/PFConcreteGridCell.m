...
{
   if ( [ self.currentSymbol isEqual: symbol_ ] )
   {
      [ self reloadDataWithSymbol: symbol_ ];
   }
   else if ( [ self.position.symbol isEqual: symbol_ ] )
   {
      [ self reloadDataWithPosition: self.position ];
   }
   else if ( [ self.order.symbol isEqual: symbol_ ] )
   {
      [ self reloadDataWithOrder: self.order ];
   }
   //!c
   //else if ( [ self.currentTrade.symbol isEqual: symbol_ ] )
   // {
   //   [ self reloadDataWithOrder: self.currentOrder ];
   // }
}*/
...
-(void)reloadDataWithLevel4Quote:( id<PFLevel4Quote> )level4_quote_
{
   [ self reloadDataWithTopText: [ NSString stringWithDouble: level4_quote_.strikePrice
                                                   precision: level4_quote_.symbol.precision ]
                     bottomText: nil ];
}
