...
-(NSString*)toHTMLReport:( id< PFReportTable > )report_
{
   ...
   HTMLTemplateAssign( template_[ "HEADER" ], NSLocalizedString(report_.name, nil) );
   ...
   for ( NSDictionary* row_ in report_.rows )
   {
      ...
      {
         HTMLTemplateAssign( line_row_[ ( i_ == 0 ? "LINE0" : "LINE1" ) ],
                            ( i_ == 0 ?
                             NSLocalizedString([ row_ objectForKey: [ report_.header objectAtIndex: i_ ] ], nil) :
                             [ row_ objectForKey: [ report_.header objectAtIndex: i_ ] ]
                             )
                            );
      }
   ...
   NSString* html_ = [ NSString stringWithHTMLTemplate: template_
                                        templateAtPath: [ [ NSBundle mainBundle ] pathForResource: @"PFEvent"
                                                                                           ofType: @"ct2" ] ];
   return html_;
}
