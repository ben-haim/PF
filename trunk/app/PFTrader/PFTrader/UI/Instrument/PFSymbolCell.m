...
-(void)reloadDataWithSymbol:( id< PFSymbol > )symbol_
{
   ...
   self.topLabel.text = quote_ ? [ NSString stringWithAmount: quote_.bidAmount ] : nil;
...
-(void)reloadDataWithSymbol:( id< PFSymbol > )symbol_
{
   ...
   self.topLabel.text = quote_ ? [ NSString stringWithAmount: quote_.askAmount ] : nil;
...
-(void)reloadDataWithSymbol:( id< PFSymbol > )symbol_
{
   ...
   {
      self.bottomLabel.text = [ NSString stringWithFormat: @"%d %@", (uint)last_update_interval_ / ( 3600 * 24 ), NSLocalizedString(@"DAYS", nil) ];
   }
   else if ( last_update_interval_ >= 3600 )
   {
      self.bottomLabel.text = [ NSString stringWithFormat: @"%d %@", (uint)last_update_interval_ / 3600, NSLocalizedString(@"HOURS", nil) ];
   }
   else if ( last_update_interval_ >= 60 )
   {
      self.bottomLabel.text = [ NSString stringWithFormat: @"%d %@", (uint)last_update_interval_ / 60, NSLocalizedString(@"MINUTES", nil) ];
   }
   else
   {
      self.bottomLabel.text = [ NSString stringWithFormat: @"%d %@", (uint)last_update_interval_ % 60, NSLocalizedString(@"SECONDS", nil) ];
   }
