...
static UIImage* UIImageFromChartMode( int chart_mode_ )
{
   switch ( chart_mode_ )
   {
      case CHART_MODE_NONE:
         return [ UIImage imageNamed: @"PFChartFree" ];
      case CHART_MODE_CROSSHAIR:
         return [ UIImage imageNamed: @"PFChartCrosshair" ];
      case CHART_MODE_SPLITTER:
         return [ UIImage imageNamed: @"PFChartSplit" ];
      case CHART_MODE_RESIZE:
         return [ UIImage imageNamed: @"PFChartMove" ];
      case CHART_MODE_DELETE:
         return [ UIImage imageNamed: @"PFChartDelete" ];
      case CHART_MODE_DRAW:
         return [ UIImage imageNamed: @"PFChartDraw" ];
   }
...
{
   self.tradesMode = [ [ quotes_ lastObject ] isMemberOfClass: [ PFTradesQuote class ] ];
...
-(void)viewDidLoad
{
   ...
   self.nameField.textColor = [ UIColor colorWithWhite: 0.78f alpha: 1.f ];
   self.nameField.borderStyle = UITextBorderStyleNone;
   self.nameField.background = [ UIImage borderedButtonBackgroundImage ];
...
didLoadTradeQuote:( id< PFLevel3Quote > )trade_quote_
{
   if ( [ self.symbol isEqual: symbol_ ] && self.tradesMode )
   {
      [ self mergePriceWithBid: trade_quote_.price
                     ...
                     AndVolume: (int)trade_quote_.size
...
-(void)reader:( id< PFQuoteFilesReader > )reader_ didLoadQuotes:( NSArray* )quotes_
{
   if ( self.reader == reader_ )
   {
      if ( self.symbol.instrument.historyType == 0 || self.symbol.instrument.historyType == 2 )
      {
