...
{
   self.spreadLabel.text = symbol_.quote
      ? [ NSString stringWithDouble: symbol_.spread precision: 1 ]
      : nil;
   
   self.volumeLabel.text = symbol_.quote
      ? [ NSString stringWithVolume: symbol_.quote.volume ]
      : nil;
}
...
-(BOOL)tryAllowTrading
{
   ...
   self.qtyField.hidden = self.qtyLabel.hidden = self.buyButton.hidden = self.sellButton.hidden = !( is_allowed_
                                                                                                    && [ [ PFSession sharedSession ] allowsMarketOperationsForSymbol: self.symbol ] );
...
-(void)viewDidLoad
{
   ...
   self.qtyField.textColor = [ UIColor colorWithWhite: 0.78f alpha: 1.f ];
   self.qtyField.borderStyle = UITextBorderStyleNone;
   self.qtyField.background = [ UIImage borderedButtonBackgroundImage ];
   self.qtyField.text = [ NSString stringWithAmount: [PFSettings sharedSettings ].lots ];
...
-(void)placeOrderWithBuySide: (BOOL)buy_side_
{
   ...
   if ( amount_ > 0.0 )
   {
      [ PFOrderEntryViewController placeMarketOrderForSymbol: self.symbol withAmount: amount_ buyMode: buy_side_ ];
   }
   else
   {
