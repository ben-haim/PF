...
-(void)viewDidLoad
{
   ...
   self.sourceRemainsItem = [ info_category_.items lastObject ];
...
{
   if ( self.transferValue > self.sourceAccount.withdrawalAvailable )
   {
...
-(void)updateTransferInfo
{
   ...
   self.sourceRemainsItem.amount = self.sourceAccount.withdrawalAvailable - self.transferValue;
...
-(void)decimalPadItemValueChanged
{
   ...
   self.targetDepositItem.amount = self.transferValue;
   [ self.tableView reloadCategory: [ self.tableView.categories objectAtIndex: 1 ] withRowAnimation: UITableViewRowAnimationNone ];
