...
{ 
    ...
    if(image!=nil)
    {
        ...
        if(isSeparatorShown)
        {
            /*CGContextSetLineWidth(ctx, CHART_BTN_BORDER_WIDTH*line1px);
            CGContextSetStrokeColorWithColor(ctx, [HEXCOLOR(border_color) CGColor]);
            CGContextMoveToPoint(   ctx,  
                                    Round_05(rect.origin.x) + rect.size.width-line1px, 
                                    rect.origin.y);
            ...
            CGContextSetStrokeColorWithColor(ctx, [HEXCOLOR(0xEFEFEFFF) CGColor]);
            CGContextMoveToPoint(   ctx,  
                                 Round_05(rect.origin.x) + rect.size.width, 
                                 rect.origin.y);
            CGContextAddLineToPoint(ctx, 
                                    Round_05(rect.origin.x) + rect.size.width, 
                                    rect.origin.y + rect.size.height-1);
