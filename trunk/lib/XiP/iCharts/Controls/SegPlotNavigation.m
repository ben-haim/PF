...
#import "SegPlotNavigation.h"



#define kCornerRadius  16.0f

@interface SegPlotNavigation (Private)
@property (nonatomic, retain, readwrite) NSMutableArray *items;
- (BOOL)mustCustomize;
@end


@implementation SegPlotNavigation
...
- (BOOL)mustCustomize {
	return true;//return self.segmentedControlStyle == UISegmentedControlStyleBordered
...
- (void)drawRect:(CGRect)rect {
	...
	CGContextMoveToPoint(c, minx - 1, midy - 1);
	CGContextAddArcToPoint(c, minx - 1, miny - 1, midx - 1, miny - 1, kCornerRadius);
	CGContextAddArcToPoint(c, maxx - 1, miny - 1, maxx - 1, midy - 1, kCornerRadius);
	CGContextAddArcToPoint(c, maxx - 1, maxy - 1, midx - 1, maxy - 1, kCornerRadius);
	CGContextAddArcToPoint(c, minx - 1, maxy - 1, minx - 1, midy - 1, kCornerRadius);
	CGContextClosePath(c);
	
	CGContextClip(c);
	
   
    CGContextSetFillColorWithColor(c, [HEXCOLOR(0xFFFFFF7F) CGColor]);
...
- (void)hide
{
    ...
    [self setFrame:CGRectMake(startFrame.origin.x, startFrame.origin.y, 1, startFrame.size.height)];
...
- (void)moveBack {
    ...
    [UIView setAnimationDuration:1.0];
    [self setFrame:CGRectMake(startFrame.origin.x, startFrame.origin.y, 200, startFrame.size.height)];
    [UIView setAnimationDelegate:self];
    [UIView setAnimationCurve: UIViewAnimationCurveEaseOut];
    [UIView commitAnimations];
}
...
{
    if ( [animID isEqualToString:@"move10"] ) {
