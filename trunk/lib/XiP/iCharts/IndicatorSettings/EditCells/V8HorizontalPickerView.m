...
- (void)layoutSubviews {
	...
	int firstNeededElement = (int)[self nearestElementToPoint:CGPointMake(offset.x, 0.0f)];
	int lastNeededElement  = (int)[self nearestElementToPoint:CGPointMake(offset.x + visibleBounds.size.width, 0.0f)];
		...
		if (!view) {
            if (i < numberOfElements) { // make sure we are not requesting data out of range
                ...
                else if (self.delegate && [self.delegate respondsToSelector:viewForElementSelector]) 
                {
                    ...
                    CGRect viewFrame = [self frameForElementAtIndex:i];
...
- (void)scrollToElement:(NSInteger)index animated:(BOOL)animate 
{
	...
	if (self.delegate && [self.delegate respondsToSelector:delegateCall] && animate) 
    {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView 
{
    ...
    if(oldSelectedIndex!=currentSelectedIndex && !noSounds)
        [XSoundPlayer PlayTock];   
