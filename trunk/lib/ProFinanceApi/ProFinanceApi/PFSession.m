...
-( id< PFCommissionLevel > )currentCommissionLevelForInstrument:( id< PFInstrument > )instrument_
{
   return [ self.spreadPlans commissionLevelWithInstrument: instrument_
                                         AndCommissionPlan: self.accounts.defaultAccount.spreadPlanId ];
}
...
{
   __weak PFSession* unsafe_session_ = self;
...
{
   NSString* message_text_ = reject_message_.comment;
...
-(BOOL)allowsMarketOperationsForSymbol:( id< PFSymbol > )symbol_
{
   return [ self.accounts.defaultAccount.tradeRouteNames containsObject: symbol_.routeName ];
}
...
{
   account_.priceForCrossPrice = [ self.spreadPlans calcSpreadBidWithBid: account_.crossInstrumentBid
                                                                     ask: account_.crossInstrumentAsk
                                                              instrument: [ self.mutableInstruments instrumentWithId: account_.crossInstrumentId ]
                                                            spreadPlanId: account_.spreadPlanId ];
}
