...
static NSUInteger version_1_6_Major = 1;
static NSUInteger version_1_6_Minor = 6;
              ...
              accountId:( int )account_id_
{
   ...
   didLoadChangePasswordStatus: 3
                     andReason: reject_message_.comment ];
       }
    } ];
}
...
-(void)scheduleClientPing
{
   ...
   ADDelayAsyncOnMainThread( ping_block_, 60.0 );
}
...
//      int max_server_field_number_ = [ (PFIntegerField*)[ message_ fieldWithId: PFFieldMaxFieldIndex ] integerValue ];
      ...
      else
      {
         ...
         NSArray* protocol_version_ = [protocol_string_ componentsSeparatedByString: @"."];
         NSUInteger v_major_ = [((NSNumber*)protocol_version_[0]) integerValue];
         NSUInteger v_minor_ = [((NSNumber*)protocol_version_[1]) integerValue];

         user_.wrongServer = (v_major_ > version_1_6_Major) || ((v_major_ == version_1_6_Major) && (v_minor_ > version_1_6_Minor));
   ...
   {
      NSString* logout_text_ = [ [ message_ fieldWithId: PFFieldText ] stringValue ];
