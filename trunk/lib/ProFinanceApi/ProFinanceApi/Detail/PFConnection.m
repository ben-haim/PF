...
-(id< PFSocket >)socket
{
   if ( !_socket )
   {
      _socket = [ PFAsyncSocketWrapper new ];
...
-(void)socket:( id< PFSocket > )socket_ didFailWithError:( NSError* )error_
{
   ...
   [ self.parseQueue async: ^(){
      ADAsyncOnMainThread(^()
                          {
                             [ self.delegate connection: self didFailWithError: error_ ];
                          });
   } ];
}
      ...
      ADAsyncOnMainThread(^()
                     {
                        [ self.delegate connection: self
                             didFailParseWithError: [ NSError PFErrorWithDescription: [ exception_ reason ] ] ];
                     });
   }
   @finally
   {
...
{
	[ self.socket sendData: data_ ];
}

-(void)sendMessage:( PFMessage* )message_
{
   [ self sendData: [ message_ data ] ];
}
