...
-(void)updateQuote:( id< PFLevel4Quote > )quote_
{
   ...
   //! to remove
   /*
   static id< ADQueue > quoteOperationsQueue = nil;
   if ( quoteOperationsQueue == nil )
   {
      quoteOperationsQueue = ADCreateSequenceQueue( @"PFLevel4QuotePackage.quoteOperationsQueue" );
   }
   
   [ quoteOperationsQueue async: ^()
    {
    } ];*/
}
