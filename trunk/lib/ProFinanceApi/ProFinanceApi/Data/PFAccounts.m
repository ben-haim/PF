...
-(id<PFAccount>)userAccount
{
   if ( !_userAccount )
   {
      PFInteger account_id_ = [ self readDefaultAccountId ];
...
-(PFAccount*)accountWithId:( PFInteger )account_id_
{
                       ...
                       delegate:( id< PFAccountsDelegate > )delegate_
{
   ...
   [ delegate_ accounts: self needCalcCrosspriceForAccount: account_ ];
}
...
-(void)recalcPammStatusAndUpdeteFromDelegate: ( id< PFAccountsDelegate > )delegate_
{
   ...
   for (PFAccount* account_ in self.accounts)
   {
      double curr_cap_ = account_.currPammCapital;
      double start_cap_ = account_.startPammCapital;
      PFPammInvestor* investor_ = [investors_ objectForKey: @(account_.accountId)];
      
      if (investor_)
      {
         account_.currPammCapital = investor_.currCapital;
         account_.startPammCapital = investor_.capital;
      }
      else
      {
         account_.currPammCapital = 0;
         account_.startPammCapital = 0;
      }
      
      if ((curr_cap_ != account_.currPammCapital) || (start_cap_ != account_.startPammCapital))
      {
         [ delegate_ accounts: self didUpdateAccount: account_ ];
      }
   }
...
-(void)addRule:( PFRule* )rule_ withTransferFinished:( BOOL )transfer_finished_
{
   ...
   PFAccount* account_ = transfer_finished_ ? [ self accountWithId: (PFInteger)rule_.accountId ] : [ self writeAccountWithId: (PFInteger)rule_.accountId ];
...
-(PFInteger)readDefaultAccountId
{
   ...
   return (PFInteger)[ account_id_ integerValue ];
}
