...
+(PFMetaObject*)metaObject
{
   return [ PFMetaObject metaObjectWithFields:
           [ NSArray arrayWithObjects: [ PFMetaObjectField fieldWithId: PFFieldId name: @"spredLevelId" ]
...
-(void)recalcLevel1Quote: (PFQuote*)quote_
{
   ...
   if ( quote_.instrument.historyType == 0 || quote_.instrument.historyType == 2 ) // By Bid or Ask
   {
      double new_open_, new_high_, new_low_, new_close_, new_last_;
      
      if ( quote_.instrument.historyType == 0 )
      {
         new_open_ = [ self calculateBidWithAsk: quote_.askOpen AndBid: quote_.open ];
         new_high_ = [ self calculateBidWithAsk: quote_.askHigh AndBid: quote_.high ];
         new_low_ = [ self calculateBidWithAsk: quote_.askLow AndBid: quote_.low ];
         new_close_ = [ self calculateBidWithAsk: quote_.askClose AndBid: quote_.previousClose ];
         new_last_ = [ self calculateBidWithAsk: quote_.ask AndBid: quote_.bid ];
      }
      else
      {
         new_open_ = [ self calculateAskWithAsk: quote_.askOpen AndBid: quote_.open ];
         new_high_ = [ self calculateAskWithAsk: quote_.askHigh AndBid: quote_.high ];
         new_low_ = [ self calculateAskWithAsk: quote_.askLow AndBid: quote_.low ];
         new_close_ = [ self calculateAskWithAsk: quote_.askClose AndBid: quote_.previousClose ];
         new_last_ = [ self calculateAskWithAsk: quote_.ask AndBid: quote_.bid ];
      }
      
      quote_.open = new_open_;
      quote_.high = new_high_;
      quote_.low = new_low_;
      quote_.previousClose = new_close_;
      quote_.last = new_last_;
   }   
...
-(void)recalcChartQuotesArrayWithQuotes: (NSArray*)quotes_
{
   ...
   {
      if ( [ [ quotes_ objectAtIndex: i ] isMemberOfClass: [ PFLotsQuote class ] ] )
      {
         PFLotsQuote* lots_qoute_ = (PFLotsQuote*)[ quotes_ objectAtIndex: i ];
         ...
         double new_askd_low_ = [ self calculateAskWithAsk: lots_qoute_.askInfo.low AndBid: lots_qoute_.bidInfo.low ];
         ...
         lots_qoute_.askInfo.low = new_askd_low_;
