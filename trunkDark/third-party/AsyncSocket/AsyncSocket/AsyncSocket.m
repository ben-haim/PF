...
- (void)closeWithError:(NSError *)err
{
	...
	if (theFlags & kDidStartDelegate)
	{
		...
		id strongDelegate = theDelegate;//retains delegate
		if ([strongDelegate respondsToSelector:@selector(onSocket:willDisconnectWithError:)])
		{
			[strongDelegate onSocket:self willDisconnectWithError:err];
		}
	}
	[self close];
}
...
- (NSError *)errorFromCFStreamError:(CFStreamError)err
{
	...
	else if(err.domain == kCFStreamErrorDomainNetDB)
	{
      ...
      message = @"SLL configuration error";
	}
...
- (void)completeCurrentRead
{
	...
	id strongDelegate = theDelegate;//retains delegate
	if([strongDelegate respondsToSelector:@selector(onSocket:didReadData:withTag:)])
	{
		[strongDelegate onSocket:self didReadData:result withTag:theCurrentRead->tag];
	}
