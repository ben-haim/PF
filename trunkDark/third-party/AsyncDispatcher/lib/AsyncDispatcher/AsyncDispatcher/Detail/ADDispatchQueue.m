...
{
   ADQueueBlock wrapped_block_ = ^void()
   {
      BOOL cancelled_ = (BOOL)dispatch_get_context( dispatch_get_current_queue() );
      if ( !cancelled_ )
      {
         block_();
      }
   };
   dispatch_async( self.queue, wrapped_block_ );
}
...
-(void)cancel
{
   if ( _queue )
   {
      dispatch_set_context( _queue, (void*)1 );
      AD_DISPATCH_RELEASE( _queue );
      self.queue = 0;
   }
}
