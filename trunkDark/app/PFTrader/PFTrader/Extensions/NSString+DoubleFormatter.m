            ...
            precision:( NSUInteger )precision_
{
   ...
   NSString* double_format_ = [ NSString stringWithFormat: @"%%0.%df", (int)precision_ ];
              ...
              symbol:( id< PFSymbol > )symbol_
{
   ...
   return [ self stringWithDouble: price_ precision: symbol_.instrument.precision ];
}
            ...
            minChange:( double )min_change_
{
   ...
   if (min_change_ != 0.0)
   {
      int symbol_precision_ = -log10( min_change_ );

      if (symbol_precision_ > 1)
         precision_ = symbol_precision_;
   }

   return [ self stringWithMoney: amount_ andPrecision: precision_ ];
}

+(id)stringWithAmount:( double )amount_
               symbol:( id< PFSymbol > )symbol_
{
    return [ self stringWithAmount: amount_ lotStep: symbol_.lotStep ];
}
