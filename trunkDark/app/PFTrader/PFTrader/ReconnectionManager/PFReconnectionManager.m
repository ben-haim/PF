...
-(void)reachabilityStatusChanged:( PFReachabilityStatus )new_status_
{
   if ( [ PFSession sharedSession ] )
   {
      if ( new_status_ == PFNoReachability )
      {
         ...
         __unsafe_unretained PFReconnectionManager* unsafe_self_ = self;
...
changePasswordForUser:( int )user_id_
{
   if ( user_id_ != 0 )
   {
      ...
      PFChangePasswordViewController* change_password_controller_ = [ PFChangePasswordViewController createAutoresetPasswordControllerWithSession: self.createdSession
                                                                                                                                     andDoneBlock: password_change_block_ ];
