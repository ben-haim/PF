...
-(NSArray*)defaultMainIndicators
{
   NSMutableArray* main_indicators_ = [ NSMutableArray new ];
   
   for ( PFIndicator* indicator_ in self.defaultIndicators )
   {
      if ( indicator_.main )
      {
         [ main_indicators_ addObject: indicator_ ];
      }
   }
   
   return main_indicators_;
}

-(NSArray*)defaultAdditionalIndicators
{
   NSMutableArray* additional_indicators_ = [ NSMutableArray new ];
   
   for ( PFIndicator* indicator_ in self.defaultIndicators )
   {
      if ( !indicator_.main )
      {
         [ additional_indicators_ addObject: indicator_ ];
      }
   }
   
   return additional_indicators_;
}
...
{
   static PFChartSettings* default_settings_ = nil;
   if ( !default_settings_ )
   {
      default_settings_ = [ self defaultSettings ];
   }
...
-(BOOL)showLoupe
{
   return [ self.properties getBoolParam: @"style.chart_general.show_loupe" ];
}

-(void)setShowLoupe:( BOOL )show_loupe_
{
   [ self.properties setParam: @"style.chart_general.show_loupe"
                    WithValue: [ NSString stringWithFormat: @"%d", show_loupe_ ] ];
}
