...
-(PFGridFooterView*)footerView
{
   if ( !_footerView )
   {
      ...
      _footerView.backgroundColor = [ UIColor gridViewFooterBackgroundColor ];
   }
   return _footerView;
}
...
-(void)setElements:( NSArray* )elements_
{
   ...
   {
      self.footerView.summaryTitle = [ NSString stringWithFormat: @"%d", (int)[ elements_ count ] ];
   }
...
viewForSelectedRowAtIndex:( NSUInteger )row_index_
{
   ...
   view_.backgroundColor = [ UIColor actionViewBackgroundColor ];
...
-(UIView*)columnsBackgroundViewInGridView:( PFGridView* )grid_view_
{
   ...
   background_view_.backgroundColor = [ UIColor clearColor ];
...
backgroundViewForRowAtIndex:( NSUInteger )row_index_
     columnAtIndex:( NSUInteger )column_index_
{
   return [ [ UIImageView alloc ] initWithImage: column_index_ == 0 ? [ UIImage  fixedCellBackgroundImage ] : [ UIImage  cellBackgroundImage ] ];
}
