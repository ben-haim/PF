...
{	 
    ...
    MIN(y_left, y_right) - parentChart.plotArea.plot_rect.origin.y < parentChart.plotArea.plot_rect.size.height &&
        ((a1.x_index>=x_start && a1.x_index > a2.x_index) || (a1.x_index<=x_end && a1.x_index < a2.x_index));
        ...
        AndCursorMode:(uint)cursorMode
{
    ...
    if(a1.y_value>a2.y_value && a2.x_index==a1.x_index)
    {
        double xCoordinate = (a1.x_index<a2.x_index)?x_right:x_left;
        CGContextAddLineToPoint(ctx, xCoordinate, y_right);
        if ((int) p1.y == (int) y_right || (int) p1.x == (int) xCoordinate) 
        {
    ...
    {
        double xCoordinate = (a1.x_index<a2.x_index)?x_right:x_left;
        double yCoordinate = (a1.x_index<a2.x_index)?y_right:y_left;
        CGContextAddLineToPoint(ctx, xCoordinate, yCoordinate);
        if ((int) p1.y == (int) yCoordinate || (int) p1.x == (int) xCoordinate) 
        {
            CGContextSetShouldAntialias(ctx, false);
        }   
        else
