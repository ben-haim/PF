...
@synthesize sumPositivePositionsNetPL;
@synthesize sumNegativePositionsNetPL;
@synthesize sumPositiveFilledOrdersNetPL;
@synthesize sumNegativeFilledOrdersNetPL;
...
-(PFAssetAccount*)assetAccountByCurrency:( NSString* )currency_
{
    ...
    {
        if ([asset_account_.currency isEqualToString: currency_])
            return asset_account_;
    }
    return nil;
}
...
{
   return self.equity == 0.0 ? 0.0 : ( self.blockedForOrders + self.initMargin ) * 100.0 / self.equity;
}

-(PFDouble)realizedPositionsNetPl
{
    return self.sumNegativePositionsNetPL + self.sumPositivePositionsNetPL;
}

-(PFDouble)realizedFilledOrdersNetPl
{
    return self.sumNegativeFilledOrdersNetPL + self.sumPositiveFilledOrdersNetPL;
}
