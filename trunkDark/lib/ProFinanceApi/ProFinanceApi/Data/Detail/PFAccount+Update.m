...
-(NSArray*)update
{
   ...
   for (PFAssetAccount* asset_account_ in self.assetAccounts)
   {
      ...
      for ( PFPosition* position_ in positions_updated_ )
      {
         ...
         double current_cross_ = [ [ PFCrossRates sharedRates ] priceForCurrency: symbol_.instrument.exp2
                                                                      toCurrency: asset_account_.currency ];
         ...
         if ((current_cross_ != -1) && (trade_session_) && (trade_session_.currentTradeSession) && (currentPrice > 0))
         {
            ...
            position_.exposure = exposure_value_;
   ...
   double sum_positive_ = 0;
   double sum_negative_ = 0;

   if (self.accountType != PFAccountTypeMultiAsset)
   {
      for ( PFPosition* position_ in positions_updated_ )
      {
         if (position_.netPl > 0)
            sum_positive_ += position_.netPl;
         else
            sum_negative_ += position_.netPl;
      }
      self.sumPositivePositionsNetPL = sum_positive_;
      self.sumNegativePositionsNetPL = sum_negative_;
   }
   ...
   if (self.accountType != PFAccountTypeMultiAsset)
   {
      for ( PFTrade* trade_ in self.trades )
      {
         if (trade_.netPl > 0)
            sum_positive_ += trade_.netPl;
         else
            sum_negative_ += trade_.netPl;
      }
      self.sumPositiveFilledOrdersNetPL = sum_positive_;
      self.sumNegativeFilledOrdersNetPL = sum_negative_;
   }

   for ( PFTrade* trade_ in [ PFSession sharedSession ].accounts.allTrades )
   {
      double current_cross_ = [ [ PFCrossRates sharedRates ] priceForCurrency: trade_.symbol.instrument.exp2
                                                                 toCurrency: trade_.account.currency ];

      trade_.exposure = trade_.price * current_cross_ * trade_.amount * trade_.symbol.instrument.lotSize;
   }
