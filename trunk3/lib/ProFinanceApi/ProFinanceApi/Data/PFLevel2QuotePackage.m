...
{
    NSMutableArray* agregated_bid_quotes_ = [ NSMutableArray new ];
    
    for ( PFLevel2Quote* quote_ in [ self.bidQuotes allObjects ] )
    {
        BOOL is_present_ = NO;
        
        for ( PFLevel2Quote* agregated_quote_ in agregated_bid_quotes_ )
        {
            if ( [ agregated_quote_ compare: quote_ ] == NSOrderedSame )
            {
                agregated_quote_.size += quote_.size;
                is_present_ = YES;
                break;
            }
        }
        ...
        if ( !is_present_ )
        {
            [ agregated_bid_quotes_ addObject: quote_ ];
        }
    }
    
    return [ agregated_bid_quotes_ sortedArrayUsingSelector: @selector(compare:) ];
}

-(NSArray*)sortedAndAgregatedAskQuotes
{
    NSMutableArray* agregated_ask_quotes_ = [ NSMutableArray new ];
    
    for ( PFLevel2Quote* quote_ in [ self.askQuotes allObjects ] )
    {
        BOOL is_present_ = NO;
        
        for ( PFLevel2Quote* agregated_quote_ in agregated_ask_quotes_ )
        {
            if ( [ agregated_quote_ compare: quote_ ] == NSOrderedSame )
            {
                agregated_quote_.size += quote_.size;
        ...
        if ( !is_present_ )
        {
            [ agregated_ask_quotes_ addObject: quote_ ];
        }
    }
    
    return [ agregated_ask_quotes_ sortedArrayUsingSelector: @selector(compare:) ];
}
