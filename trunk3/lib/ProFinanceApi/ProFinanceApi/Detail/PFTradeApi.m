...
//   NSLog( @"Trade Message: %@", message_ );
   ...
   {
      [ self.delegate api: self didLoadCommissionPlan: [ PFCommissionPlan objectWithFieldOwner: message_ ] ];
   }
   else if ( message_type_ == PFMessageSpreadPlan )
   {
      [ self.delegate api: self didLoadSpreadPlan: [ PFSpreadPlan objectWithFieldOwner: message_ ] ];
   }
   else if ( message_type_ == PFMessageAssetType )
   {
       [ self.delegate api: self didLoadAssetType: [ PFAssetType objectWithFieldOwner: message_ ] ];
   }
   else if ( message_type_ == PFMessageChangePassword )
   {
      [ self. delegate api: self didLoadChangePasswordStatus: [ [ message_ fieldWithId: PFFieldChangePasswordStatus ] byteValue ] andReason: nil ];
   }
   else if ( message_type_ == PFMessageTradeSession )
   {
      [ self.delegate api: self didLoadTradeSessionContainer: [ PFTradeSessionContainer objectWithFieldOwner: message_ ] ];
   }
   else if ( message_type_ == PFMessageOvernightMarginNotification )
   {
      [ self.delegate api: self overnightNotificationForAccountId: [ (PFIntegerField*)[ message_ fieldWithId: PFFieldAccountId ] integerValue ]
          maintanceMargin: [ [ message_ fieldWithId: PFFieldMaintanceMargin ] doubleValue ]
          availableMargin: [ [ message_ fieldWithId: PFFieldAvailableMargin ] doubleValue ]
                     date: [ [ message_ fieldWithId: PFFieldDate ] dateValue ] ];
   }
   else
   {
