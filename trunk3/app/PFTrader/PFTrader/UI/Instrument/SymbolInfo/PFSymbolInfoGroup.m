...
{
   NSMutableArray* margin_rows_array_ = [ NSMutableArray new];
   BOOL use_long_short_margin_ = self.symbol.instrument.useLongShortMargin;
   BOOL use_overnight_margin_ = self.symbol.instrument.useOvernightMargin;
   PFInstrumentMarginType margin_type_ = self.symbol.instrument.marginType;

   NSArray* margin_coefficients_ = self.symbol.instrument.marginCoefficients;
   PFMarginCoefficient* curr_margin_coef_;

   if (self.symbol.instrument.marginCoefficients.count > 0)
      curr_margin_coef_ = [self.symbol.instrument.marginCoefficients objectAtIndex: 0];
   else
      NSLog(@"Margin is empty");

   if (self.symbol.instrument.isDerivative)
   {
      for (PFMarginCoefficient* margin_coef_ in margin_coefficients_)
      {
         if ((self.symbol.expDay   == margin_coef_.expDay  ) &&
             (self.symbol.expMonth == margin_coef_.expMonth) &&
             (self.symbol.expYear  == margin_coef_.expYear ))
         {
            curr_margin_coef_ = margin_coef_;
         }
      }
   }

   if (!use_long_short_margin_ && !use_overnight_margin_)
   {
      double init_margin_call_size_;
      double maint_margin_call_size_;

      if (margin_type_ == PFInstrumentMarginTypePFS_ACC)
      {
         init_margin_call_size_  = [PFSession sharedSession].accounts.defaultAccount.tradingLevel;
         maint_margin_call_size_ = [PFSession sharedSession].accounts.defaultAccount.marginLevel;
      }
      else
      {
         init_margin_call_size_  = curr_margin_coef_.initMarginCallSize;
         maint_margin_call_size_ = curr_margin_coef_.holdMarginCallSize;
      }

      [ margin_rows_array_ addObject: [ PFSymbolInfoRow infoRowWithName: NSLocalizedString( @"SYMBOL_INFO_MARGIN_REQ", nil )
                                                               andValue: [self getMarginStrFromType: margin_type_
                                                                                         initMargin: init_margin_call_size_
                                                                                     andMaintmargin: maint_margin_call_size_] ] ];
   }
   else if (!use_long_short_margin_ && use_overnight_margin_)
   {
      [ margin_rows_array_ addObject: [ PFSymbolInfoRow infoRowWithName: NSLocalizedString( @"SYMBOL_INFO_DAY", nil )
                                                               andValue: [self getMarginStrFromType: margin_type_
                                                                                         initMargin: curr_margin_coef_.initMarginCallSize
                                                                                     andMaintmargin: curr_margin_coef_.holdMarginCallSize] ] ];

      [ margin_rows_array_ addObject: [ PFSymbolInfoRow infoRowWithName: NSLocalizedString( @"SYMBOL_INFO_OVERNIGHT", nil )
                                                               andValue: [self getMarginStrFromType: margin_type_
                                                                                         initMargin: curr_margin_coef_.initMarginCallSizeOvernight
                                                                                     andMaintmargin: curr_margin_coef_.holdMarginCallSizeOvernight] ] ];
   }
   else if (use_long_short_margin_ && !use_overnight_margin_)
   {
      [ margin_rows_array_ addObject: [ PFSymbolInfoRow infoRowWithName: NSLocalizedString( @"SYMBOL_INFO_BUY", nil )
                                                               andValue: [self getMarginStrFromType: margin_type_
                                                                                         initMargin: curr_margin_coef_.initMarginCallSize
                                                                                     andMaintmargin: curr_margin_coef_.holdMarginCallSize] ] ];

      [ margin_rows_array_ addObject: [ PFSymbolInfoRow infoRowWithName: NSLocalizedString( @"SYMBOL_INFO_SELL", nil )
                                                               andValue: [self getMarginStrFromType: margin_type_
                                                                                         initMargin: curr_margin_coef_.initMarginCallSizeShort
                                                                                     andMaintmargin: curr_margin_coef_.holdMarginCallSizeShort] ] ];
   }
   else
   {
      [ margin_rows_array_ addObject: [ PFSymbolInfoRow infoRowWithName: NSLocalizedString( @"SYMBOL_INFO_DAY_BUY", nil )
                                                               andValue: [self getMarginStrFromType: margin_type_
                                                                                         initMargin: curr_margin_coef_.initMarginCallSize
                                                                                     andMaintmargin: curr_margin_coef_.holdMarginCallSize] ] ];

      [ margin_rows_array_ addObject: [ PFSymbolInfoRow infoRowWithName: NSLocalizedString( @"SYMBOL_INFO_DAY_SELL", nil )
                                                               andValue: [self getMarginStrFromType: margin_type_
                                                                                         initMargin: curr_margin_coef_.initMarginCallSizeShort
                                                                                     andMaintmargin: curr_margin_coef_.holdMarginCallSizeShort] ] ];

      [ margin_rows_array_ addObject: [ PFSymbolInfoRow infoRowWithName: NSLocalizedString( @"SYMBOL_INFO_OVERNIGHT_BUY", nil )
                                                               andValue: [self getMarginStrFromType: margin_type_
                                                                                         initMargin: curr_margin_coef_.initMarginCallSizeOvernight
                                                                                     andMaintmargin: curr_margin_coef_.holdMarginCallSizeOvernight] ] ];

      [ margin_rows_array_ addObject: [ PFSymbolInfoRow infoRowWithName: NSLocalizedString( @"SYMBOL_INFO_OVERNIGHT_SELL", nil )
                                                               andValue: [self getMarginStrFromType: margin_type_
                                                                                         initMargin: curr_margin_coef_.initMarginCallSizeOvernightShort
                                                                                     andMaintmargin: curr_margin_coef_.holdMarginCallSizeOvernightShort] ] ];
   }

   return margin_rows_array_;
}

-(id)getMarginStrFromType:( int )margin_type_
               initMargin:( double )init_margin_
           andMaintmargin:( double )maint_margin_
{
   BOOL is_percentage_ = (margin_type_ != PFInstrumentMarginTypeFutures);

   if (is_percentage_ && (margin_type_ != PFInstrumentMarginTypePFS_ACC))
   {
      init_margin_  *= (margin_type_ == PFInstrumentMarginTypeSetlemen) ? 1            : 100;
      maint_margin_ *= (margin_type_ == PFInstrumentMarginTypeSetlemen) ? init_margin_ : 100;
   }

   NSString* str_init_margin_;
   NSString* str_maint_margin_;

   if (is_percentage_)
   {
      str_init_margin_  = [[NSString stringWithAmount: init_margin_]  stringByAppendingString: @"%"];
      str_maint_margin_ = [[NSString stringWithAmount: maint_margin_] stringByAppendingString: @"%"];
   }
   else
   {
      str_init_margin_  = [NSString stringWithFormat: @"%@ %@", [NSString stringWithAmount: init_margin_],  self.symbol.instrument.exp2];
      str_maint_margin_ = [NSString stringWithFormat: @"%@ %@", [NSString stringWithAmount: maint_margin_], self.symbol.instrument.exp2];
   }

   return [NSString stringWithFormat: @"%@ / %@", str_init_margin_, str_maint_margin_];
}

-(NSString*)commissionNameByType: (int)type_
{
   switch (type_) {
      case PFCommissionTypePerLot:
         return NSLocalizedString(@"SYMBOL_INFO_FEES_PER_LOT", nil);

      case PFCommissionTypePerFill:
         return NSLocalizedString(@"SYMBOL_INFO_FEES_PER_FILL", nil);

      case PFCommissionTypePertransaction:
         return NSLocalizedString(@"SYMBOL_INFO_FEES_PER_TRANSACTION", nil);

      case PFCommissionTypePerphonetransaction:
         return NSLocalizedString(@"SYMBOL_INFO_FEES_PER_PHONE_TRANSACTION", nil);

      case PFCommissionTypeVat:
         return NSLocalizedString(@"SYMBOL_INFO_FEES_VAT", nil);

      case PFCommissionTypePercent:
